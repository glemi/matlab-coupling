function [Z, data] = HBAR_v2(f, varargin)
    parameters = varargin;
    f = [f(:)]; %#ok
    ...
    layer('Piezo', 'ASN15',500e-9, Acntr, parameters);
    ...
    C0     = Ccntr*Cring/(Ccntr+Cring) + abs(Cd);   %Series connection
    
    ZC0    = 1./(2i*pi*f*C0);
    Rel    = TopEl.material.Resistivity*TopEl.thickness/Acntr ... % what?
            +BotEl.material.Resistivity*BotEl.thickness/Aring + abs(Rs);    % is this right?
    ...
    data = var2struct(f, gamma, kt2, Ztop, Zbot, C0, ZC0, Rel);
