function [data, zfit] = bvdMultiFit(f, z, n, varargin)
    ...
    if any(i)
        ...
        initial = recycle([], f, z);
        %                   fr   dfr  Qm   Rm    C0     Rs
         lb = initial - v2c(10e3, 2e3, 300, 100, 0.2e-12, 1);
         ub = initial + v2c(10e3, 2e3, 300, 100, 0.2e-12, 1);
    else
        initial = coeff_estimate(f, z, n); 
        lb = [];
        ub = [];
    end
    opts    = optimoptions('lsqcurvefit', 'Display', 'off', varargin{:});
    ...
    if ~any(isnan(initial))
        if debug_plot
            ...
            [final, ~,~,~,~ ] = lsqcurvefit(@(c,f)wrapper(n,c,f), initial, logf, log(abs(z)), lb, ub, opts);
...
function prepareCoeffPlot()
    ...
    line(NaN, NaN,'DisplayName', 'f_r', 'Color', colors(1,:));
    line(NaN, NaN,'DisplayName', '\Delta{}f_r', 'Color', colors(2,:));
    line(NaN, NaN,'DisplayName', 'Q_m', 'Color', colors(4,:));
    line(NaN, NaN,'DisplayName', 'R_m', 'Color', colors(3,:));
    line(NaN, NaN,'DisplayName', 'C_0', 'Color', colors(5,:));
    line(NaN, NaN,'DisplayName', 'R_s', 'Color', colors(6,:));
