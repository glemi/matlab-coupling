function model_output_analysis_v3(parameter, values, parName, parUnit, parScale, axScale)
    opt parUnit char '';
    ...
    for k = 1:n 
        ...
        arg_P   = {'tPiezo', tPiezo, 'tSubst', 0,      'tOxide', 0,      'tBotEl', 0,      'tTopEl', 0};
        arg_PE  = {'tPiezo', tPiezo, 'tSubst', 0,      'tOxide', 0,      'tBotEl', tBotEl, 'tTopEl', tTopEl};
        arg_PS  = {'tPiezo', tPiezo, 'tSubst', tSubst, 'tOxide', tOxide, 'tBotEl', 0,      'tTopEl', 0  };
        arg_PSE = {'tPiezo', tPiezo, 'tSubst', tSubst, 'tOxide', tOxide, 'tBotEl', tBotEl, 'tTopEl', tTopEl};

        [Z_P(:,k),   par_FP(k)  ]  = HBAR_v3(f, cfile, arg_P{:},   arg_var{:});
        [Z_PE(:,k),  par_FPE(k) ]  = HBAR_v3(f, cfile, arg_PE{:},  arg_var{:});
        [Z_PS(:,k),  par_FPS(k) ]  = HBAR_v3(f, cfile, arg_PS{:},  arg_var{:});
        [Z_PSE(:,k), par_FPSE(k)]  = HBAR_v3(f, cfile, arg_PSE{:}, arg_var{:});
    ...
    fname = sprintf('modeloutput_v3:%s', parameter);
    fig(fname); clf;
