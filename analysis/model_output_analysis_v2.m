function model_output_analysis_v2(parameter, values, parName, parUnit, parScale, axScale)
    opt parUnit char '';
    ...
    for k = 1:n 
        ...
        [Z_P(:,k),  par_FP(k)  ]  = HBAR_v2(f, arg_P{:},   arg_var{:});
        [Z_PE(:,k), par_FPE(k) ]  = HBAR_v2(f, arg_PE{:},  arg_var{:});
        [Z_PS(:,k), par_FPS(k) ]  = HBAR_v2(f, arg_PS{:},  arg_var{:});
        [Z_PSE(:,k),par_FPSE(k)]  = HBAR_v2(f, arg_PSE{:}, arg_var{:});
    ...
    fname = sprintf('modeloutput_v2:%s', parameter);
    fig(fname); clf;
